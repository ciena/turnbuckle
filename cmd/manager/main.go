...
type configSpec struct {
	...
	DefaultBindingEvaluationInterval time.Duration
	ShowVersion                      bool
...
func main() {
	...
	flag.DurationVar(&config.OfferEvaluationInterval,
		"offer-evaluation-interval", 10*time.Second,
	...
	flag.DurationVar(&config.OfferEvaluationErrorInterval,
		"offer-evaluation-error-interval", 1*time.Minute,
		"The interval until an offer is re-evaluated when an error is "+
			"seen during its processing")
	flag.DurationVar(&config.DefaultBindingEvaluationInterval,
		"default-binding-evaluation-interval", 30*time.Second,
		"The interval to recheck binding compliance if not set on the binding")
	...
	if err = (&controllers.ConstraintPolicyOfferReconciler{
		Client:                       mgr.GetClient(),
		Scheme:                       mgr.GetScheme(),
		OfferEvaluationInterval:      config.OfferEvaluationInterval,
		OfferEvaluationErrorInterval: config.OfferEvaluationErrorInterval,
	}).SetupWithManager(mgr); err != nil {
